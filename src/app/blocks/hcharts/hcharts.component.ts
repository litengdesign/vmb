...
export class HchartsComponent implements OnChanges, OnInit {
  ...
  public StartTime: any = format(addDays(new Date(), -30), 'YYYY-MM-DD');
  public EndTime: any = format(new Date(), 'YYYY-MM-DD');
  ...
  public datepicker = [format(addDays(this.today, -30), 'YYYY-MM-DD'),
  ...
  constructor(private router: ActivatedRoute, public server: ServersService, private http: HttpClient) { }
  ngOnInit() {}
  //判断组件绑定值变化
  ngOnChanges(changes: SimpleChanges) {
    if (changes['searchData'] && changes['searchData'].currentValue) {
      ...
      if(currentDate.length){
        this.StartTime = format(currentDate[0], 'YYYY-MM-DD');
        this.EndTime = format(currentDate[1], 'YYYY-MM-DD');
