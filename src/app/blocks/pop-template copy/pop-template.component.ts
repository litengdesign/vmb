...
export class PopTemplateComponent implements OnInit {
  ...
  public TidalMonth:any = startOfMonth(new Date())
  defaultSelectDate = [
  ...
  getAlert(reset: boolean = false){
      ...
      let options = {
        ...
        params: {
          ...
          Id: this.popData.deviceId,
          StartTime: this.alertOption.defaultSelectDate[0],
          EndTime: this.alertOption.defaultSelectDate[1],
        }
      }
      this.server.getRxjsData(options).subscribe((data) => {
  ...
  getTidelList(reset: boolean = false,item?){
    ...
    this.server.getRxjsData(options).subscribe((data) => {
      ...
      if(this.selectedTidal.name=='潮位月报表'){
        ...
        let StartDateTime = startOfMonth(this.TidalMonth);
        let EndDateTime = endOfMonth(this.TidalMonth);
